= ppozon's Responses to Ruby Koans

These are my responses to the Ruby Koans, uploaded for version control and 
backup. I've written a Greed Game, the extra credit assignment, which uses
rand for dictating the actions of the players. It is run through the
command-line through the regular Ruby Koans CLI. The game play is shown
on the terminal's stdout.
