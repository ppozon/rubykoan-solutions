...
class AboutMessagePassing < Neo::Koan
  ...
  def test_all_messages_are_caught
    ...
    assert_equal "Someone called irrigate with <>", catcher.irrigate
    assert_equal "Someone called wineglass with <1>", catcher.wineglass(1)
    assert_equal "Someone called aguirre with <1, 2, 3, 4, 5, 6>", catcher.aguirre(1,2,3,4,5,6)
  end

  def test_methods_that_invoke_methods_missing_are_still_undefined
    catcher = AllMessageCatcher.new
  ...
  class WellBehavedFooCatcher
    # one can implement a dynamic undefined method name dispatcher
    def method_missing(method_name, *args, &block)
      if method_name.to_s[0,3] == "mak"
        "Mak to you too"
      else
  ...
  def test_foo_method_are_caught
    ...
    assert_equal "Mak to you too", catcher.mak_bar
    assert_equal "Mak to you too", catcher.mak_baz
  end
  ...
  class WellBehavedFooCatcher
    # one can redefine respond_to? to pretend that the caught undefined
    # methods are really defined
  ...
  def test_explicitly_implementing_respond_to_lets_objects_tell_the_truth
    ...
    assert_equal true, catcher.respond_to?(:mak_bar)
