...
class Proxy
  ...
  def messages
    str = @messages.inspect
# puts " ----> Called MESSAGES = [ #{str} ]"
    @messages
  end

  def method_missing(method_name, *args, &block)
# puts " ----> Method called and forwarding: #{method_name} #{args} #{block} "
    @messages.push(method_name)
    @object.send(method_name, *args, &block)
  end

  def called?(method_name)
# puts " ----> Called called? for method: #{method_name} "
    @messages.include?(method_name) 
  end

  def number_of_times_called(method_name)
    str = @messages.inspect
# puts " ----> number_of_times_called for #{method_name} with #{str}"
    @messages.select { |a| a == method_name }.count
  end
end
